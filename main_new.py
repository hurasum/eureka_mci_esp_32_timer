    ...
    def __getEncoderValue(self):
        ...
        while True:
            ...
            if ntf == _thread.EXIT:
                ...
                return
            self.value = self.encoder_state_machine.value()
            time.sleep(0.001)
...
class States:
        ...
        while True:
            ...
            if not self.edit_state:
                ...
                if self.state == self.single.STATE:
                    self.single.seconds = value
                elif self.state == self.double.STATE:
                    self.double.seconds = value
    ...
    def editState(self):
        ...
        print(self.state)
        if self.state == self.endless.STATE:
            self.edit_state = False
            self.pin_menu.init(trigger=self.uc.Pin.IRQ_FALLING)
            return
        if self.edit_state:
            value = 0.1
            if self.state_old == self.single.STATE:
                value = self.single.SECONDS
            elif self.state_old == self.double.STATE:
                value = self.double.SECONDS
            value *= 20
            self.encoder_state_machine.setTime(value)
        else:
            self.encoder_state_machine.setState(self.state_old)
        self.pin_menu.init(trigger=self.uc.Pin.IRQ_FALLING)
    ...
    def editCPS(self):
        ...
        while True:
            ...
            text = "CPS:   {}\r".format(round(self.cps, 3))
...
class CoffeeGrinder:
    ...
    def __getShotTimes(self):
        ...
        if single_sec:
            self.states.single.seconds = single_sec / 20
        if double_sec:
            self.states.double.seconds = single_sec / 20
        if cps:
    ...
    def setEdit(self, pin):
        ...
        elif not self.states.edit_state:
            self.states.edit_state = True
    ...
    def __sleepTime(self, sleep):
        ...
        while end_time < sleep:
            text = "Seconds: {}".format(round(sleep, 3))
